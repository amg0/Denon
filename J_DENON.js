...
var DENON = (function(api,$) {
	...
	function DENON_Settings(deviceID) {
		function _buildTypesSelect(id,selected) {
			var result = DENON.format('<select id="{0}" class="form-control netmon-select-type">{1}</select>',id,
				jQuery.map(types, function(type) {
					return DENON.format('<option {1}>{0}</option>',type,(type,selected==type) ? 'selected' : '')
				}))
			return result
		};
		function _buildTargetLineHtml(target) {
			var type = _buildTypesSelect('netmon-type',target.type)
			var name = DENON.format("<input class='form-control' id='netmon-name' value='{0}' required></input>",target.name)
			var ipaddr = DENON.format("<input class='form-control' id='netmon-ipaddr' value='{0}' required></input>",target.ipaddr || "")
			var page = DENON.format("<input class='form-control {1}' id='netmon-page' value='{0}'></input>",target.page || "", (target.type=='http') ? '' : 'hidden d-none')
			var btn_del = DENON.format(btnTemplate,'netmon-del','Delete','fa fa-trash-o text-danger','btn-sm netmon-del')
			return DENON.format('<tr><td>{0}</td> <td>{1}</td> <td>{2}</td> <td>{3}</td> <td>{4}</td> </tr>',
				name,
				type,
				ipaddr,
				page,
		...
		var mytargets = JSON.parse(get_device_state(deviceID,  DENON.DENON_Svs, 'Targets',1))
		var types = JSON.parse( get_device_state(deviceID,  DENON.DENON_Svs, 'Types',1))

		jQuery.each( map, function( idx, item) {
			var value = (item.value!=undefined) ? item.value : get_device_state(deviceID,  DENON.DENON_Svs, item.variable,1)
			var editor = ""
			if (item.variable==undefined && item.value==undefined) {
				editor = DENON.format("<button id='{0}' class='btn btn-secondary btn-sm'>{1}</button>", item.id, item.label)
			} else if (item.value==undefined) {
				editor = DENON.format("<input class='form-control' id='{0}' value='{1}'></input>",item.id, value)
			} else {
				editor = DENON.format("<input class='form-control' value='{0}' disabled></input>",value)
			}
			fields.push( 
				DENON.format('<tr><td>{0}</td><td>{1}</td></tr>',
					DENON.format("<label for='{0}'><b>{1}</b> :</label>",item.id,item.label),
					editor ) 
				)
		});
		
		html += DENON.format("<h3>Parameters</h3><table class='table table-responsive table-sm'><thead>{0}</thead><tbody>{1}</tbody></table>",
			headings,
			fields.join("\n") 
		);
		...
		var btn_add = DENON.format(btnTemplate,'add','Add','fa fa-plus','netmon-add')
		...
		function _onSave(e) {
			...
			if (form.checkValidity() === false) {
				...
				jQuery.each(map, function(idx,item) {
					...
					DENON.saveVar(deviceID,  DENON.DENON_Svs, item.variable, varVal, false)
				});
				var targets = []
				...
				DENON.saveVar(deviceID,  DENON.DENON_Svs, 'Targets', JSON.stringify(targets),(ui5==false))
